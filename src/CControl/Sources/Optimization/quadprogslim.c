...
static STATUS_CODES optislim(const float Q[], const float c[], const float A[],
 ...
 * 1969, Wismer and Chattergy, 1978. The algorithm is slightly changed and even
 * more optimized by Daniel Mårtensson(2025) in both size and speed. Min
 ...
 * x [column_a]				// Solution
 */
STATUS_CODES quadprogslim(const float Q[], const float c[], const float A[],
  ...
  if (equality_constraints_are_used) {
    ...
    const STATUS_CODES status =
        optislim(Q, c, A_long, b_long, x, row_a + row_g + row_g, column_a);
...
static void linsolve_upper_tran_triangular(const float A[], float x[],
                                           const float b[], const size_t row) {
  int32_t i, j;
  for (i = row - 1; i >= 0; i--) {
    /* Start with b[i] and substract the values that already has been solved */
    x[i] = b[i];

    /* A[j * row] because A is lower triangular but is interpreted as upper
     * triangular */
    for (j = i + 1; j < row; j++) {
      x[i] -= A[i + j * row] * x[j];
    }
