...
static STATUS_CODES opti(const float Q[], const float c[], const float A[],
                         const float b[], float x[], const size_t row_a,
                         const size_t column_a);

/*
 * This is quadratic programming with optimized Hildreth's method by Luenberger,
 * 1969, Wismer and Chattergy, 1978. The algorithm is slightly changed and very
 * much optimized by Daniel Mårtensson(2025) in both size and speed. Min
 * 1/2x^TQx + c^Tx S.t Ax <= b Gx = h
 *
 * If you want to do maximization, then turn Q and c negative. The constraints
 * are the same
 *
 * Max 1/2x^T(-Q)x + (-c)^Tx
 * S.t Ax <= b
 ...
 * x [column_a]			// Solution
 */
STATUS_CODES quadprog(const float Q[], const float c[], const float A[],
                      const float b[], const float G[], const float h[],
                      float x[], const size_t row_a, const size_t row_g,
                      const size_t column_a,
                      const bool equality_constraints_are_used) {
  if (equality_constraints_are_used) {
    /* Create multiple inequality constraints.Those are going to be equality
     * constranits */
    float *A_long =
        (float *)malloc((row_a + row_g + row_g) * column_a * sizeof(float));
    ...
    const STATUS_CODES status =
        opti(Q, c, A_long, b_long, x, row_a + row_g + row_g, column_a);
...
static STATUS_CODES opti(const float Q[], const float c[], const float A[],
  ...
  /* Use Cholesky factorization to solve x from Qx = c because Q is square and
   * symmetric */
  ...
  /* Solve QP = A' (Notice that we are using a little trick here so we can avoid
   * A') */
...
/*static STATUS_CODES opti(const float Q[], const float c[], const float A[],
const float b[], float x[], const size_t row_a, const size_t column_a);

THIS IS ACTUALLY NEWER CODE THAN ABOVE. This code below gives the same result as
above, but take less memory. The drawback is that this code below is much slower
than the code above! 2025-02-15
*
* This is quadratic programming with optimized Hildreth's method by Daniel
Mårtensson
* Min 1/2x^TQx + c^Tx
* S.t Ax <= b
*     Gx = h
...
STATUS_CODES quadprog(const float Q[], const float c[], const float A[], const
float b[], const float G[], const float h[], float x[], const size_t row_a,
const size_t row_g, const size_t column_a, const bool
equality_constraints_are_used) { if (equality_constraints_are_used) {
               // Create multiple inequality constraints. Those are going to be
equality constranits float* A_long = (float*)malloc((row_a + row_g + row_g) *
column_a * sizeof(float)); float* A_long0 = A_long; float* b_long =
(float*)malloc((row_a + row_g + row_g) * sizeof(float)); float* b_long0 =
b_long;
               ...
               // Optimize
               const STATUS_CODES status = opti(Q, c, A_long, b_long, x, row_a +
row_g + row_g, column_a);
...
const float b[], float x[], const size_t row_a, const size_t column_a) {
       ...
       // Use Cholesky factorization to solve x from Qx = c because Q is square
and symmetric if (!linsolve_chol(Q, x, c, column_a)) { return STATUS_NAN;
       }
       ...
       // Solve QP = A' (Notice that we are using a little trick here so we can
avoid A') float* P = (float*)malloc(row_a * column_a * sizeof(float)); for (i =
0; i < row_a; i++) { if (!linsolve_chol(Q, P + i * column_a, A + i * column_a,
column_a)) { free(P); return STATUS_NAN;
               }
       }
       ...
       for (i = 0; i < MAX_ITERATIONS; i++) {
               ...
               for (j = 0; j < row_a; j++) {
                       ...
                       for (k = 0; k < row_a; k++) {
                               // Compute H
                               const float H = dot(A + k * column_a, P + j *
column_a, column_a);
...
*
       ...
       % S.t Ax <= b
       %
       % If you want to do maximization, then turn Q and c negative. The
constraints are the same
       %
       % Max 1/2x^T(-Q)x + (-c)^Tx
       % S.t Ax <= b
       %
       % Input: Q(Symmetric matrix), c(Objective function), A(Constraint
matrix), b(Constraint vector) % Output: x(Solution vector), solution(boolean
flag) % Example 1: [x, solution] = mc.quadprog(Q, c, A, b) % Author: Daniel
Mårtensson 2022 September 3
